...
async function parseFDAFile(filePath) {
  ...
  return new Promise((resolve, reject) => {
    fs.createReadStream(filePath)
      ...
      .on('data', (row) => {
        const deviceName = row['col8']?.trim(); // brand name
        const deviceProblem = row['col9']?.trim();
        const patientProblem = row['col10']?.trim();
        const eventText = row['col14']?.trim();

        if (!deviceName || !deviceProblem) return;

        const severity = classifySeverity(`${deviceProblem} ${patientProblem} ${eventText}`);
        rawEvents.push({ deviceName, deviceProblem, patientProblem, eventText, severity });
      })
      .on('end', () => {
        const deviceMap = new Map();

        for (const event of rawEvents) {
          const key = event.deviceName;
          if (!deviceMap.has(key)) {
            deviceMap.set(key, {
              deviceName: event.deviceName, // ✅ Renamed here
